...
import type { SwiperData, imageData, SwiperItem, EditObject } from '../../types/swiper'
...
const AddData = ({ ...pros }: addDataProps): React.JSX.Element => {
    ...
    const [imgDat, setImgData] = useState<imageData>([])
const{inx, flag} = editObj
    useEffect(()=>{             
        if(inx!==null){
            const corrData = data[inx];
            setFormData(corrData);
    ...
    return (
        <div>
            <h5 className="mt-3 text-base/7 font-semibold text-gray-900">Add Data:</h5>
